...
@app.route("/")  # temp for now
@app.route("/home")
def homepage():
    return render_template("homepage.html", logged_user=forms.LOGGED_USER)


@app.route("/about")
def about():
    return render_template("about.html", logged_user=forms.LOGGED_USER)
...
def home():
    ...
    if forms.LOGGED_USER != 'empty':
        ...
        results = conn.execute(
            'SELECT * FROM UserRestaurantReviews WHERE UserName = "{}";'.format(forms.LOGGED_USER)).fetchall()
        conn.close()
...
@app.route("/newRestaurants", methods=['GET', 'POST'])
def newRestaurants():
...
def update_review(ReviewNumber):
    ...
    if form.validate_on_submit():
        ...
        commit = conn.execute("Update UserRestaurantReviews SET Review = '{}', MealCost = '{}', ConvenienceRating = '{}', FoodRating = '{}', RestaurantName = '{}' WHERE ReviewNumber = '{}';".format(
            form.content.data, form.meal_cost.data, form.convenience_rating.data, form.food_rating.data, form.title.data, ReviewNumber))
...
def delete_review(ReviewNumber):
    conn = db.connect()
    conn.execute(
        "DELETE FROM UserRestaurantReviews WHERE ReviewNumber = '{}';".format(ReviewNumber))
    conn.close()
...
def search_review():
    ...
    if form.validate_on_submit():
        conn = db.connect()
        results = conn.execute(
            "SELECT * FROM UserRestaurantReviews WHERE RestaurantName = '{}';".format(form.restaurant_name.data)).fetchall()
        count = conn.execute("SELECT COUNT(RestaurantName) FROM UserRestaurantReviews WHERE RestaurantName = '{}' GROUP BY RestaurantName;".format(
            form.restaurant_name.data)).fetchall()
        if len(results) == 0:
